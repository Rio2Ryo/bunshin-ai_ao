...
CLAUDE_BIN=/home/ginocbot/.npm-global/bin/claude
...
1. **タスクごとにブランチを切る**: `feat/<task-name>` で作業開始（main に直接コミット禁止）
2. **実装した機能の Playwright E2E テストを書く**: 既存テストファイルに追加 or 新規作成
3. **ブランチにコミット & push**: テストの実行は待たなくてよい（バックグラウンドで自動実行される）
4. **コミットメッセージ規約**: `feat: ...` / `fix: ...` / `test: ...`

テストは別プロセスが自動実行し、パスしたら次タスクへ進む。
失敗したら修正タスクが自動生成される。

## テスト品質基準（必須）

実装した機能に対して、以下の全カテゴリのテストを書くこと。「ページが表示される」だけのテストは不十分。

### 正常系
- 主要なユーザーフローを最初から最後まで通す（登録→操作→結果確認）
- 各ステップで期待する画面要素・データが存在することを assert

### 異常系・バリデーション
- 未入力・空送信でエラーメッセージが表示されること
- 不正な入力（長すぎる文字列、特殊文字、SQLインジェクション的文字列）でクラッシュしないこと
- 存在しないIDへのアクセスで404/エラーが返ること

### 境界値・エッジケース
- 0件データ時の表示（空リスト、「データがありません」表示）
- 大量データ時の動作（ページネーションが機能するか）
- 連続クリック・二重送信でデータが重複しないこと

### 権限・セキュリティ
- 未ログインユーザーが保護ページにアクセスできないこと（リダイレクト確認）
- 他テナント・他ユーザーのデータにアクセスできないこと
- 管理者専用機能に一般ユーザーがアクセスできないこと

### UI・インタラクション
- モバイルビューポートでレイアウトが崩れないこと
- ボタン・リンクがクリック可能であること
- フォーム入力→送信→結果反映の一連フロー

テストは「この機能を本番に出して大丈夫か？」を判断する品質で書くこと。

## 作業メモ（agent-memory.md）— 必須・随時更新

`~/agent-memory.md` はセッション間の記憶引き継ぎファイル。**ローテーション時にまとめて書くのではなく、作業中に随時更新すること。**

### 更新タイミング
- ファイルを新規作成・修正したとき
- 設計判断をしたとき（なぜその方法を選んだか）
- エラーに遭遇して解決したとき（原因と対処法）
- テストを書いた/実行したとき（結果と残課題）
- コミットしたとき（ハッシュと変更内容）

### 書き方
- 箇条書きで簡潔に、ただし **具体的に**（ファイル名、関数名、エラーメッセージを含める）
- 「○○を実装した」ではなく「`src/routes/matching.ts` の `calculateScore()` にストリークボーナス加算ロジック追加、3日+10pt/7日+30pt/30日+100pt」
- 時系列で追記していく（上書きしない）
- 未解決の問題は `TODO:` プレフィックスで明記

**これはセッションの生存に関わるルール。省略禁止。**
